...
def predict_student(request):
    ...
    try:
        ...
        # NEW: Generate personalized schedule
        schedule = generate_schedule(hours, attendance, sleep, stress, exercise)
...
def generate_schedule(hours, attendance, sleep, stress, exercise):
    """Generate personalized weekly schedule based on student metrics."""
    base_schedule = {
        "Monday": ["7-8 AM: Wake up & hydrate", "8-9 AM: Breakfast", "9 AM-12 PM: Classes"],
        "Tuesday": ["7-8 AM: Wake up & stretch", "8-9 AM: Breakfast", "9 AM-12 PM: Classes"],
        "Wednesday": ["7-8 AM: Wake up & hydrate", "8-9 AM: Breakfast", "9 AM-12 PM: Classes"],
        "Thursday": ["7-8 AM: Wake up & stretch", "8-9 AM: Breakfast", "9 AM-12 PM: Classes"],
        "Friday": ["7-8 AM: Wake up & hydrate", "8-9 AM: Breakfast", "9 AM-12 PM: Classes"],
        "Saturday": ["8-9 AM: Wake up", "9-10 AM: Breakfast", "10 AM-1 PM: Deep Study"],
        "Sunday": ["8-9 AM: Wake up", "9-10 AM: Breakfast", "10 AM-12 PM: Review Week"],
    }
    
    daily_study_slots = max(2, int(hours))
    if hours < 3:
        for day in base_schedule:
            base_schedule[day].extend([
                f"2-3 PM: Study Block 1 ({daily_study_slots}h goal)",
                "4-5 PM: Study Block 2",
                "7-8 PM: Evening Review"
            ])
    if attendance < 75:
        for day in base_schedule:
            base_schedule[day][0] = "6:30 AM: Prep & attend all classes (target 90%)"
    if sleep < 7:
        for day in base_schedule:
            base_schedule[day].append("10 PM: Bedtime (8h sleep goal)")
    if stress > 7:
        for day in base_schedule:
            base_schedule[day].insert(-2, "6-6:30 PM: Meditation/Breathing exercises")
    if exercise < 3:
        exercise_days = ["Monday", "Wednesday", "Friday"]
        for day in exercise_days:
            base_schedule[day].append("5-6 PM: 30min walk/jog")
...
@csrf_exempt
def save_feedback(request, student_id):
    if request.method == "POST":
